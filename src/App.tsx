...
import { GasProtocolData, INITIAL_COMPONENTS, GasComponent, COMPONENT_OPTIONS, BRAND_KITS } from './types';
...
export default function App() {
  ...
  // 1. Подготвяме празен масив, базиран на първоначалните компоненти
  let updatedComponents = INITIAL_COMPONENTS.map(comp => ({
    ...comp,
    make: "",
    approvalNumber: ""
  }));

  // 2. Проверяваме дали избраната марка съществува в нашите готови комплекти
  const selectedKit = BRAND_KITS[brand];

  if (selectedKit) {
    // Ако има такъв кит, минаваме през всеки начален компонент и търсим данни за него
    updatedComponents = updatedComponents.map((baseComp) => {
      // Търсим в кита по име (с изчистване на интервали за всеки случай)
      const kitItem = selectedKit.find(
        (item) => item.name?.trim() === baseComp.name.trim()
      );
  ...
  // 3. Обновяваме състоянието (state)
  setData((prev) => ({
    ...prev,
